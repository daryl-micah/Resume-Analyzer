...
connection = pymysql.connect(host='localhost',user='root',password='Darylryan@1',db='ra_project')
...
def run():
    ...
    img = Image.open('./RA_Logo.png')
    img = img.resize((250,250))
    st.image(img)

    # Create the DB
    db_sql = """CREATE DATABASE IF NOT EXISTS ra_project;"""
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'raAdmin' and ad_password == 'raAdmin123':
                st.success("Welcome!")
                # Display Data
